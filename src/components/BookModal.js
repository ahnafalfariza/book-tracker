...
import { addToLibrary } from '../utils/skynet'
import ReactLoading from 'react-loading'
import Modal from './Modal'

const BookModal = ({ bookData, onClose }) => {
  const { userId, booksData, setBooksData } = useStore()
  ...
  const isBookInLibrary = () => {
    return booksData.findIndex((x) => x.id === book.id)
  }

  const onChangeStatus = async (value) => {
    setReadingStatusLoader(value)
    const newData = [...booksData]
    const newBook = { ...bookData, libraryType: value }
  ...
  return (
    <Fragment>
      <Modal close={(_) => onClose()}>
        <div className="mx-auto max-w-md w-full h-full max-h-screen py-6 p-4">
          <div className="w-full h-full relative bg-dark-primary-800 rounded-md shadow-md overflow-hidden pt-4 pb-12">
            ...
            <Scrollbars autoHide className="opacity-100">
              <div className="px-4">
                ...
                <div className="text-center mt-4">
                  ...
                  <p className="mt-1 opacity-75">{book.authors[0]}</p>
                </div>
                <div>
                  ...
                  <div className="mt-2 flex justify-between">
                    ...
                    <p>{book.publishedDate?.split('-')[0] || 'Not Available'}</p>
                  </div>
                  <div className="mt-2 flex justify-between">
                    <p className="opacity-75">ISBN</p>
                    <p>{book.isbn && book.isbn[0].identifier}</p>
                  </div>
                  <div className="mt-2 flex justify-between">
                  ...
                  <div className="mt-2 flex justify-between">
                    ...
                    <p>{book.categories || 'Not available'}</p>
                  </div>
                </div>
              </div>
            </Scrollbars>
            <div className="absolute z-10 bottom-0 left-0 right-0">
              {isBookInLibrary(book.id) > -1 && userId ? (
                ...
                >
                  {booksData[isBookInLibrary(book.id)].libraryType === 'readingList' && (
                    <div className="flex items-center">
                      <div className="bg-gray-100 rounded-full w-6 h-6 p-1 flex items-center justify-center">
                        <svg
                          width="100%"
                          height="100%"
                          viewBox="0 0 24 24"
                      ...
                      <p className="ml-4 text-lg font-medium">Reading List</p>
                    </div>
                  )}
                  {booksData[isBookInLibrary(book.id)].libraryType === 'readingNow' && (
                    <div className="flex items-center">
                      <div className="bg-gray-100 rounded-full w-6 h-6 p-1 flex items-center justify-center">
                        <svg
                          width="100%"
                      ...
                      <p className="ml-4 text-lg font-medium">Reading Now</p>
                    </div>
                  )}
                  {booksData[isBookInLibrary(book.id)].libraryType === 'finished' && (
                    <div className="flex items-center">
                      <div className="bg-gray-100 rounded-full w-6 h-6 p-1 flex items-center justify-center">
                        <svg
                          width="100%"
                      ...
                      <p className="ml-4 text-lg font-medium">Finished</p>
                    </div>
                  )}
                </button>
              ) : (
                ...
                >
                  {userId ? (
                    <p className="ml-4 text-lg font-medium">Add to Library</p>
                  ) : (
                    <p className="ml-4 text-lg font-medium">Login to Add to Library</p>
                  )}
                </button>
              )}
            </div>
      ...
      {showModal === 'addToLibrary' && (
        <Modal close={(_) => setShowModal(null)} closeOnEscape={false}>
          <div className="max-w-md w-full mx-auto p-4">
            <div className="bg-dark-primary-800 p-2 rounded-md">
              <div>
                  ...
                  >
                    <div
                      className={`flex items-center opacity-50 ${
                        (book.libraryType === 'readingList' || readingStatusLoader === 'readingList') && 'opacity-100'
                      }`}
                    >
                      <div className="bg-gray-100 rounded-full w-8 h-8 p-1 flex items-center justify-center">
                        <svg
                          width="100%"
                          height="100%"
                          viewBox="0 0 24 24"
                ...
                <div className="w-full my-1">
                  ...
                  >
                    <div
                      className={`flex items-center opacity-50 ${
                        (book.libraryType === 'readingNow' || readingStatusLoader === 'readingNow') && 'opacity-100'
                      }`}
                    >
                      <div className="bg-gray-100 rounded-full w-8 h-8 p-1 flex items-center justify-center">
                        <svg
                ...
                <div className="w-full my-1">
                  ...
                  >
                    <div
                      className={`flex items-center opacity-50 ${
                        (book.libraryType === 'finished' || readingStatusLoader === 'finished') && 'opacity-100'
                      }`}
                    >
                      <div className="bg-gray-100 rounded-full w-8 h-8 p-1 flex items-center justify-center">
                        <svg
                ...
                {isBookInLibrary() > -1 && (
                  <div className="w-full my-1">
                    <div
                      onClick={(_) => onChangeStatus('remove')}
                      className="flex justify-between cursor-pointer px-2 py-2 rounded-md hover:bg-dark-primary-900"
                    >
                      <div
                        className={`flex items-center opacity-50 ${readingStatusLoader === 'remove' && 'opacity-100'}`}
                      >
                        <div className="bg-red-500 rounded-full w-8 h-8 p-1 flex items-center justify-center">
                          <svg
                            width="100%"
                            height="100%"
                            viewBox="0 0 24 24"
                          ...
                          >
                            <path
                              fill-rule="evenodd"
                              clip-rule="evenodd"
                              d="M5 1H15.4142L21 6.58579V21C21 22.1046 20.1046 23 19 23H5C3.89543 23 3 22.1046 3 21V3C3 1.89543 3.89543 1 5 1ZM13 3H5V21H19V9H15C13.8954 9 13 8.10457 13 7V3ZM15 3.41421V7H18.5858L15 3.41421ZM8.29289 11.7071L9.70711 10.2929L12 12.5858L14.2929 10.2929L15.7071 11.7071L13.4142 14L15.7071 16.2929L14.2929 17.7071L12 15.4142L9.70711 17.7071L8.29289 16.2929L10.5858 14L8.29289 11.7071Z"
                              fill="black"
                            />
                          </svg>
                        </div>
                        <p className="ml-4 text-xl text-red-500">Remove</p>
                      </div>
                      {readingStatusLoader === 'remove' && (
                        <div className="w-8">
                          <ReactLoading color="#f56565" height="100%" width="100%" />
                        </div>
                      )}
                    </div>
                  </div>
